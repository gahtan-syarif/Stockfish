...
Value Eval::evaluate(const Eval::NNUE::Networks& networks, const Position& pos, int optimism) {
    ...
    const auto adjustEval = [&](int optDiv, int nnueDiv, int nnueConstant, int optConstant, 
                                int evalDiv, int shufflingConstant, int shufflingDiv) {
        // Blend optimism and eval with nnue complexity and material imbalance
        optimism += optimism * (nnueComplexity + std::abs(simpleEval - nnue)) / optDiv;
        nnue -= nnue * (nnueComplexity * 5 / 3) / nnueDiv;

        int npm = pos.non_pawn_material() / 64;
        v       = (nnue * (npm + nnueConstant) + optimism * (optConstant + npm)) / evalDiv;
    ...
    if (!smallNet)
        adjustEval(513, 32395, 919, 145, 1036, 178, 204);
    else if (psqtOnly)
        adjustEval(517, 32857, 908, 155, 1019, 224, 238);
    else
        adjustEval(499, 32793, 903, 147, 1067, 208, 211);
